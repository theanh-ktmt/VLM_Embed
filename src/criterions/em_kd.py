...
class EMKDLoss(nn.Module):
    """
    Exact Matching Knowledge Distillation (EMKD) Loss.
    
    This loss performs distillation by aligning student and teacher embeddings using
    optimal transport (linear sum assignment) to match vision tokens to text tokens
    or vision tokens to vision tokens.
    """

    def __init__(self, args: Any):
        """
        Initializes the EMKDLoss module.

        Args:
            args: Configuration arguments containing weights for KD loss.
        """
    ...
    def forward(self, distiller: nn.Module, input_data: Dict[str, Any]) -> Dict[str, Tensor]:
        """
        Forward pass to compute the total loss including Contrastive Loss and EMKD Loss.

        Args:
            distiller: The distillation wrapper.
            input_data: Dictionary containing inputs.
        ...
        # Calculate number of text tokens (approximate check based on vocab range if generic, 
        # but here it uses specific IDs likely for image tokens or special tokens)
        ...
        for i in range(batch_size):
            logger.debug(f"Sample {i}: num_text_qry_tokens {num_text_qry_tokens[i]}, num_text_pos_tokens {num_text_pos_tokens[i]}")
            logger.debug(f"Sample {i} input_ids ids of teacher {teacher_qry_input['input_ids'][i]}, pos {teacher_pos_input['input_ids'][i]}")
            logger.debug(f"Sample {i} input_ids ids of student {student_qry_input['input_ids'][i]}, pos {student_pos_input['input_ids'][i]}")
            ...
            if student_qry_image_features is not None and teacher_qry_image_features is not None:
                if cur_idx_qry_img < len(student_qry_image_features) and cur_idx_qry_img < len(teacher_qry_image_features):
                    if student_qry_image_features[cur_idx_qry_img] is not None and teacher_qry_image_features[cur_idx_qry_img] is not None:
                        ...
                        logger.debug(f"Sample qry {i}: num_tokens_vision_qry_stu {num_tokens_vision_qry_stu}, num_tokens_vision_qry_tea {num_tokens_vision_qry_tea}")
                        ...
                        logger.debug(f"Sample qry {i}: student_qry_vision_hidden_state shape {student_qry_vision_hidden_state.shape}, "
                                     f"teacher_qry_vision_hidden_state shape {teacher_qry_vision_hidden_state.shape}, "
                                     f"student_qry_text_hidden_state shape {student_qry_text_hidden_state.shape}, "
                                     f"teacher_qry_text_hidden_state shape {teacher_qry_text_hidden_state.shape}")
                        logger.debug(f"Sample qry {i}: student_qry_hidden_state shape {student_qry_hidden_states[-1][i].shape}, "
                                     f"teacher_qry_hidden_state shape {teacher_qry_hidden_states[-1][i].shape}")
                        ...
                        # Linear Assignment for Vision features
                        c_match = torch.sum(
                        ...
                        logger.debug(f"Sample qry {i}: affinity_s shape {affinity_s.shape}, affinity_t shape {affinity_t.shape}")

                        loss_vlad += F.smooth_l1_loss(affinity_s, affinity_t)
                        cur_idx_qry_img += 1

            # --- Positive Side Processing ---
            if student_pos_image_features is not None and teacher_pos_image_features is not None:
                if cur_idx_pos_img < len(student_pos_image_features) and cur_idx_pos_img < len(teacher_pos_image_features):
                    if student_pos_image_features[cur_idx_pos_img] is not None and teacher_pos_image_features[cur_idx_pos_img] is not None:
                        ...
                        logger.debug(f"Sample pos {i}: num_tokens_vision_pos_stu {num_tokens_vision_pos_stu}, num_tokens_vision_pos_tea {num_tokens_vision_pos_tea}")
                        ...
                        logger.debug(f"Sample pos {i}: student_pos_vision_hidden_state shape {student_pos_vision_hidden_state.shape}, "
                                     f"teacher_pos_vision_hidden_state shape {teacher_pos_vision_hidden_state.shape}, "
                                     f"student_pos_text_hidden_state shape {student_pos_text_hidden_state.shape}, "
                                     f"teacher_pos_text_hidden_state shape {teacher_pos_text_hidden_state.shape}")
                        logger.debug(f"Sample pos {i}: student_pos_hidden_state shape {student_pos_hidden_states[-1][i].shape}, "
                                     f"teacher_pos_hidden_state shape {teacher_pos_hidden_states[-1][i].shape}")
                        ...
                        logger.debug(f"Sample pos {i}: affinity_s shape {affinity_s.shape}, affinity_t shape {affinity_t.shape}")
                        loss_vlad += F.smooth_l1_loss(affinity_s, affinity_t)
                        cur_idx_pos_img += 1

        loss_distill = (0.25 * loss_vsd + 25 * loss_vlad) / batch_size

        loss_mse = 0.5 * (nn.MSELoss()(student_qry_reps, self.distiller.projectors["t2s"](teacher_qry_reps)) +
                          nn.MSELoss()(student_pos_reps, self.distiller.projectors["t2s"](teacher_pos_reps)))
